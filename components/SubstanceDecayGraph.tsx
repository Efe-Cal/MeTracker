...
function ToolTip({ x, y, yVal }: { x: SharedValue<number>; y: SharedValue<number>; yVal: Readonly<SharedValue<string>> }) {
    return (
    <>
        <Circle cx={x} cy={y} r={4} color="black"/>
        <SKText x={x} y={x} text={yVal} font={useFont(require("@/assets/fonts/calibri.ttf"),20)}/>
    </>
    );
}
...
function MyCustomLine({ points }: { points: PointsArray }) {
    ...
    return <Path path={path} style="stroke" strokeWidth={3} color="black"/>;
}

export default function SubstanceDecayGraph({intakes, halflife}:SDProps) {
    ...
    return (
        <View style={{
            ...
            <Text style={{fontSize: 20}}>Current amount: ~{currentAmount} mg</Text>
            <Canvas style={{ width: 400, height: 50 }}>
                <SKText text={value} font={useFont(require("@/assets/fonts/calibri.ttf"),20)} x={10} y={20} />
            </Canvas>
            <ScrollView horizontal={true} showsHorizontalScrollIndicator={false} contentOffset={{x:intakes[0]?(2000*timeToNumber(intakes[0].time)/48)-50:0,y:0}}>
                ...
                >
                    {({ points }) => (
                        <>
                            <MyCustomLine points={points.amount} />
                            {isActive ? (
                                <ToolTip x={state.x.position} y={state.y.amount.position} yVal={value} />
                            ) : null}
